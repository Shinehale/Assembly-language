...
int  getBiggerComb( Player* ptrPlayer, Comb* ptrTargetComb );
...
int main()
{
    printf("\nPoker Game version 5: One player play and others follow \n");
...
void prepareGame( int allCards[], Player players[] )
{
    ...
    for ( i = 0, ptrPlayer = players; i < PLAYER_NUM; i ++, ptrPlayer ++ )
    {
        ...
        ptrPlayer->faceNum=countCards( ptrPlayer->cardIdPlan, ptrPlayer->cardFaceStat );
    }
}
...
//
// Play the game, one player start and other players follow

void playGame( Player players[] )
{
    Comb lst;
    int  pos;
    while (players[0].cardNum!=0){
        pos=getNextComb(&players[0]);
        playOneComb(&players[0],pos);
        lst=players[0].combArray[pos];
        pos=getBiggerComb(&players[1],&lst);
        playOneComb(&players[1],pos);
        if (pos>=0) lst=players[1].combArray[pos];
        pos=getBiggerComb(&players[2],&lst);
        playOneComb(&players[2],pos);
        printf(" \n");
    }
}
