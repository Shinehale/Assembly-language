...
void planCombs( Player* ptrPlayer )
{
    ...
    for (int i=0;i<ARRAY_COMB_SIZE;i++) ptrPlayer->used[i]=0;
    ...
    while (getpriGroup(ptrPlayer->cardIdPlay,ope,&new_,8)){
    	ptrPlayer->combArray[tot]=new_;
        tot++;
	}
    while (getpriGroup(ptrPlayer->cardIdPlay,ope,&new_,7)){
    	ptrPlayer->combArray[tot]=new_;
        tot++;
	}
	while (getpriGroup(ptrPlayer->cardIdPlay,ope,&new_,6)){
    	ptrPlayer->combArray[tot]=new_;
        tot++;
	}
    for (int i=11;i>=5;i--){
...
int  getNextComb( Player* ptrPlayer )
{
	for (int i=0;i<ptrPlayer->combNum;i++)
        if (ptrPlayer->used[i]==0&&ptrPlayer->combArray[i].typeId!=4){
            return i;
        }
	for (int i=0;i<ptrPlayer->combNum;i++)
        if (ptrPlayer->combArray[i].typeId==4){
            return i;
        }
    return -1;
}
...
int  getBiggerComb(Player* ptrPlayer, Comb* ptrTargetComb, int *needUpdate)
{
    for (int i=0;i<ptrPlayer->combNum;i++)
        if (ptrPlayer->used[i]==0&&ptrPlayer->combArray[i].typeId==ptrTargetComb->typeId){
            ...
            if (ptrPlayer->combArray[i].typeId==1){
            	if (ptrTargetComb->maxFace==2) return -1;
            	if (ptrPlayer->combArray[i].maxFace==2) return i;
            	if (ptrPlayer->combArray[i].maxFace==1) {
            		if (ptrTargetComb->maxFace!=1) return i;
            		else continue;
				}
				if (ptrTargetComb->maxFace==1){
					if (ptrPlayer->combArray[i].maxFace!=2) continue;
					else return i;
				}
				if (ptrPlayer->combArray[i].maxFace>ptrTargetComb->maxFace) return i;
			}
			if (ptrPlayer->combArray[i].typeId==2){
            	if (ptrTargetComb->maxFace==2) return -1;
            	if (ptrPlayer->combArray[i].maxFace==2) return i;
            	if (ptrPlayer->combArray[i].maxFace==1) {
            		if (ptrTargetComb->maxFace!=1) return i;
			...
			if (ptrPlayer->combArray[i].typeId==3){
            	if (ptrTargetComb->maxFace==2) return -1;
            	if (ptrPlayer->combArray[i].maxFace==2) return i;
            	if (ptrPlayer->combArray[i].maxFace==1) {
            		if (ptrTargetComb->maxFace!=1) return i;
            ...
            if (ptrPlayer->combArray[i].typeId==6){
            	if (ptrTargetComb->maxFace==2) return -1;
            	if (ptrPlayer->combArray[i].maxFace==2) return i;
            	if (ptrPlayer->combArray[i].maxFace==1) {
            		if (ptrTargetComb->maxFace!=1) return i;
			...
			if (ptrPlayer->combArray[i].typeId==7){
            	if (ptrTargetComb->maxFace==2) return -1;
            	if (ptrPlayer->combArray[i].maxFace==2) return i;
            	if (ptrPlayer->combArray[i].maxFace==1) {
            		if (ptrTargetComb->maxFace!=1) return i;
			...
			if (ptrPlayer->combArray[i].typeId==8){
            	if (ptrTargetComb->maxFace==2) return -1;
            	if (ptrPlayer->combArray[i].maxFace==2) return i;
            	if (ptrPlayer->combArray[i].maxFace==1) {
            		if (ptrTargetComb->maxFace!=1) return i;
